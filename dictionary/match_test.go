...
func Test_Match(t *testing.T) {
	...
	t.Run("must be able to find word without offset", func(t *testing.T) {
		...
		require.True(t, ok)
		require.Equal(t, "orange", doc.Value)
	})
	t.Run("must be able to find word with offset", func(t *testing.T) {
		...
		doc, ok := dict.docRaw(m.ToArray()[0])
		require.True(t, ok)
		require.Equal(t, "ranger", doc.Value)
	})
}
